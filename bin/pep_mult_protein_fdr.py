...
def main():
  ...
  percTabBases = list()
  if dataSet == "prest":
    #percTabFolder = "/media/storage/mergespec/data/FE_Kall_vial_v2_%d/percolator_tdc/tab/"
    #percTabBase = os.path.join(percTabFolder, "FE_Kall_vial_v2_%d.percolator")
    #for i in range(1,4):
...
def writeProteinFdrs(percTabBase, picked):
  ...
  if picked:
    targetOutFN = percTabBase + ".tab.pepmult_picked_proteins"
    decoyOutFN = percTabBase + ".decoys.tab.pepmult_picked_proteins"
  else:
    targetOutFN = percTabBase + ".tab.pepmult_classic_proteins"
    decoyOutFN = percTabBase + ".decoys.tab.pepmult_classic_proteins"
  ...
  for row in reader:
    if len(row) == 6:
      if row[5] not in proteinQvalueMap:
        ...
        proteinQvalueMap[row[5]][0] *= float(row[3])
